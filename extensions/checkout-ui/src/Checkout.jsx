...
function ProductReview() {
  ...
  if (buyerJourney.shippingAddress !== undefined) {
    ...
    if (
      allowed_country.includes(shipping_Country) ||
      allowed_country.includes(billing_Country)
    ) {
      ...
      useEffect(() => {
        ...
        if (
          ...
          if (
            ...
            !ids.every((id, index) => id === productIds[index])
          ) {
            setProductIds(
              ids
            ); /* Update product IDs only if they are different */
          }
        }
      ...
      const skipAvTags = allTags.filter(
        (tag) => tag.toLowerCase() === "skip_av"
      ).length;
